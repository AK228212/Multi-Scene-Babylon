...
export default class Player extends AbstractPlayer {
  ...
  public game: GardenScene | CastleScene;
  ...
  public update() {
    ...
    const deltaTime = this.game._scene.getAnimationRatio();
    ...
    if (Config.debugPlayer) {
      this.mesh.ellipsoidMesh.position = this.mesh
        .get()
        .position.add(this.mesh.get().ellipsoidOffset);
    }
  }
  ...
  private setHorizontalMovement(
    moveVector: Vector3,
    currMoveDir: MoveDirection,
    onGround: boolean
  ) {
    let moveDir;
    if (onGround) {
