...
export default class PlayerMesh {
  ...
  public async build() {
    this.mesh = MeshBuilder.CreateCylinder(this.name, {
      height: this.height,
      diameter: this.width,
    });
  ...
  public isOnGround(verticalOffset = 0): boolean {
    let onGround = false;
    const compareWith =
      this.mesh.getBoundingInfo().minimum.y +
  ...
  public isOnCeiling(verticalOffset = 0): boolean {
    let onCeiling = false;
    const compareWith =
      this.mesh.getBoundingInfo().maximum.y +
      this.mesh.position.y +
      verticalOffset;
  ...
  private drawCollisionEllipsoid() {
    ...
    ellipsoidMesh.position = this.mesh
      .getAbsolutePosition()
      .add(this.mesh.ellipsoidOffset);
