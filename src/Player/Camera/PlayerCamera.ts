...
const CAMERA_CROUCH_OFFSET =
  CAMERA_STAND_OFFSET * AbstractPlayer.CROUCH_Y_SCALING;
...
export default class PlayerCamera {
  ...
  private setupFirstPersonCamera() {
    this.firstPersonCamera = new UniversalCamera(
      ...
      this.player.level._scene
    );
    this.firstPersonCamera.attachControl(this.player.game._canvas, true);
  ...
  public update() {
    ...
    this.firstPersonCamera.position.set(
      pos.x,
      pos.y + this.cameraOffset,
      pos.z
    );

    this.thirdPersonCamera.alpha =
      THIRD_PERSON_ALPHA_OFFSET - this.firstPersonCamera.rotation.y;
    this.thirdPersonCamera.beta =
      THIRD_PERSON_BETA_OFFSET - this.firstPersonCamera.rotation.x;
  }
